...
chatsRouter.post('/:id/messages', (req, res) => {
    ...
    const userIndex = newDb.users.findIndex(
        (userDb) => userDb.id === req.body.userId,
    )
    const otherUserIndex = newDb.users.findIndex(
        (userDb) => userDb.id === req.body.otherUserId,
    )
    ...
    let otherUserChatIndex = currentOtherUser.chats.findIndex((chatDb) =>
        chatDb.participants.includes(req.body.userId),
    )
    ...
    if (!chatId) {
        ...
        if (userChatIndex !== -1) {
            Object.assign(currentUser.chats[userChatIndex], userNewChat)
        } else {
